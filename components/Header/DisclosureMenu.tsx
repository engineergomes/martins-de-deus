  ...
  return (
    <>
      <Disclosure>
        {({ open }) => (
          <>
            <Disclosure.Button className="flex items-center text-[#424530] text-lg font-semibold transition duration-300 ease-linear px-6">
              ...
              {items.map((item) => (
                <div className="" key={item.name}>
                  <Link href={item.href} as={item.as}>
                    <button
                      className={` group flex w-full items-center rounded-md px-2 py-2 text-base text-[#424530] 
					  text-md font-regular transition duration-200 ease-linear `}
                      onClick={() => {
                        onClick();
                      }}
                    >
