  ...
  return (
    <>
      <div className="relative">
        <div id="ver-mais" className="absolute -top-28 bg-transparent"></div>
        <div className="flex flex-col max-w-[1360px] mx-auto gap-10 items-center justify-center px-5">
          ...
          <div className="hidden lg:grid grid-cols-4 gap-10 place-items-center">
            {direitoDaFamilia.map((item, index) => (
              <div
                key={index}
                className="grid grid-cols-1 grid-rows-[1fr,1fr,1fr] gap-0 rounded p-5 lg:p-10 w-full h-full place-items-center bg-[white] text-[#414530] fill-[#d5a675] border-4 border-transparent hover:border-[#d5a675] drop-shadow hover:drop-shadow-xl transition duration-300 ease-in-out"
              >
                <div className="h-16 w-16 lg:h-24 lg:w-24 fill-[#d5a675]">
                  {item.icon}
                </div>
